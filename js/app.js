...
var FEATURE_CATALOG = [
  {
    ...
    items: [
      ...
      { name: '三合/飞星/四化 模式切换', desc: '三合盘（三方四正）· 飞星盘（宫干四化飞星+动画脉冲）· 四化盘（禄权科忌高亮+非四化星曜半透明）' },
      { name: '星曜亮度7级颜色编码', desc: '庙(金)·旺(暗金)·得(浅金)·利(灰金)·平(灰)·不(浅红)·陷(红)，一眼识别星曜吉凶' }
    ]
  },
  {
  ...
  {
    icon: '🤖', title: 'AI 大模型对话', tag: 'ai-llm',
    items: [
      { name: '结构化命盘导出', desc: '一键导出完整命盘为 Markdown 文本，含十二宫位/星曜/四化/格局/神煞/八字，可直接粘贴到任何 AI 聊天工具' },
      { name: '多供应商支持', desc: 'DeepSeek（推荐·免费额度）· OpenAI (GPT-4o-mini) · 通义千问，三种 AI 后端自由切换' },
      { name: 'AI 实时对话面板', desc: '底部滑出式聊天窗口，基于命盘数据与 AI 实时对话，支持追问和上下文记忆' },
      { name: '8 个快捷问题', desc: '财运分析·事业发展·婚姻感情·健康运势·性格深度·格局解读·大限走势·人际关系，一键提问' },
      { name: 'API 配置面板', desc: '图形化配置界面，支持连接测试，配置持久化到 localStorage' },
      { name: '复制命盘文本', desc: '一键复制结构化命盘数据到剪贴板，可用于外部 AI 工具' },
      { name: '对话历史', desc: '支持多轮对话上下文记忆，可清空历史重新开始' }
    ]
  },
  {
    icon: '📊', title: '数据可视化图表', tag: 'chart-analysis',
    items: [
      { name: '十二宫能量雷达图', desc: '6层同心网格 + 十二宫评分多边形，直观展示各宫吉凶强弱对比' },
      { name: '大限走势折线图', desc: '12大限宫位能量波动曲线，填充色区分高低，预测人生起伏趋势' },
      { name: '五行分布饼图', desc: '14主星按木火土金水五行归类，扇形比例着色，一目了然' },
      { name: '吉煞比例环形图', desc: '吉星 vs 煞星占比环形图，中心显示吉星百分比' },
      { name: '星曜分布热力图', desc: '12宫 × 5维度（主星/辅星/吉星/煞星/四化）热度矩阵，直观定位密集区域' },
      { name: '纯 Canvas 实现', desc: '零外部依赖，高性能渲染，支持响应式缩放' }
    ]
  },
  {
    icon: '🤖', title: 'AI 智能解读（规则引擎）', tag: 'ai',
    items: [
  ...
  },
  {
    ...
    items: [
      ...
      { name: '叠宫显示', desc: '本命/大限/流年三重宫位叠加，四色图例区分（本命金·大限蓝·流年绿·流月橙）' },
  ...
  },
  {
    ...
    items: [
      ...
      { name: '详情面板', desc: '命盘概要 + 🤖AI解读 + 📥导出 + 🔗合盘 + 📐三盘同参 + 📊数据图表 + 💬AI对话' },
